 ...
 * @param owner    이 줄의 주인. 요청자가 자기 줄을 <b>전부</b> 알아보려면 신원이 필요하다 —
 *                 등수나 어템프트 ID로 견주면 최고 기록 한 줄만 자기 것으로 표시된다.
 *                 응답에는 나가지 않는다(남의 사용자 ID·세션 ID를 공개 조회로 흘리면 안 된다)
 * @param nickname 주인이 로그인 사용자일 때의 닉네임. 게스트면 null이라 표시 이름은 세션 ID로 만든다
