...
final class CarryLineSessions {
    ...
    /**
     * 깊은 라운드 확보 레버. 스모크에서 후반(3라운드 이후) 편집 표본이 전체 편집의 10% 미만이면
     * 각 세션의 마지막 프롬프트를 이 상수로 갈아 끼우고 다시 돌린다.
     *
     * <p><b>스위치가 아니라 코드 교체다.</b> {@code build.gradle}을 건드리지 않기로 했으므로
     * 새 {@code -P} 속성을 받을 수 없다. 교체하면 그 사실을 설계 문서에 기록한다.
     */
    static final String N_T4_DEEPER =
            "취소된 주문 목록을 조회하는 기능도 만들어 줘. 취소 사유도 함께 저장하고 보여 줘.";
    static final String C_T4_DEEPER =
            "OrderController에 취소와 조회를 둘 다 넣고 응답 형식을 정리해 줘. OrderController만 고쳐.";
