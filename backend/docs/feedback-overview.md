...
| **작업 방식** (코드 전역에서 `pattern`, purpose는 `PATTERN_FEEDBACK`) | 이 턴에 일한 방식의 이름과 그 근거. 필요하면 `쓸 기법` | 세션 하나의 이름과 다음 세션에 가져갈 기법 |
...
지금은 답을 세션이 고른다. 어느 프롬프트에도 돌려 본 이야기가 없으면 `automated check`, 돌려 본
이야기는 있는데 코드를 짚은 적이 없으면 `human review`다.
...
용어 표기는 `원어 — 한국어 풀이`이고 **원어는 절대 번역하지 않는다.** 사용자가 다음 세션에 가져가는
것이 그 영어 이름이기 때문이다.
...
| 작업 방식 총평 | `### 이번 세션의 이름`(+ 그 이름을 받치는 턴 번호)과 `### 다음 세션에 가져갈 것` | 세션 이름과 같은 용어. `vibe coding`은 가져갈 것이 될 수 없다 |
