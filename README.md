...
> **그림과 다른 곳** — `WebSocket` 선이 `TURN Server`에 닿아 있지만, 실제로 그 소켓은 백엔드와 연결됩니다. coturn은 음성 중계만 맡습니다. Python 워커는 그림에 둘이지만 실제로는 하나입니다. 채점 결과는 워커가 백엔드를 직접 부르는 것이 아니라 결과 큐를 거치고 백엔드가 그것을 소비합니다.
