...
> **그림과 다른 곳** — 백엔드 상자의 `MyBatis`는 실제로는 **jOOQ**입니다. 결과 큐에서 DB로 가는 화살표(`Save Result`)와 문제 저장소에서 DB로 가는 화살표는 둘 다 백엔드를 거칩니다(8번, 그리고 위 폴링 문단). 시그널링은 `TURN Server`가 아니라 백엔드가 처리하고, coturn은 음성 중계만 맡습니다. Python 워커는 그림에 둘이지만 실제로는 하나입니다. `CI/CD Pipeline`과 `QA / Test Automation` 두 구역은 계획이고 아직 구현되어 있지 않습니다.
