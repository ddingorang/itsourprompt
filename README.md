...
프롬프트를 한 번 보내는 것이 **턴** 하나입니다. 턴이 쌓이면서 코드의 **빌드업**이 이루어집니다. 
...
<img src="docs/screenshots/08-attempt-grading.png" alt="사진 설명" style="height: 500px; object-fit: cover; object-position: top;">
...
1. 사용자가 프롬프트를 보낸다.
2. **게이트가 먼저 본다.** 
    - 무엇을 원하는지 읽어낼 수 없으면 여기서 거절되고 코드 생성 호출로 넘어가지 않는다.
3. 백엔드가 LLM을 호출한다. 
...
7. 컨테이너가 요청을 소비(consume)하여 자기 컨테이너에서 컴파일하고 테스트를 돌린 뒤, 결과를 결과 큐로 돌려보낸다.
