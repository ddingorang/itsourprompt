...
「모두의 프롬프트」는 그 계기를 인위적으로 만듭니다. 대충 물어보면 원하는 코드가 나오지 않는 조건을 걸어두고, 그 안에서 문제를 풀게 합니다.
...
**배포**는 앞단에 Nginx를 두고 컨테이너로 돌립니다. 배포용 compose와 CI 설정은 이 저장소에서 관리하지 않습니다. Jenkins·ECR 파이프라인과 QA 자동화(E2E → TestRail → Jira)는 계획이고 아직 구현되지 않았습니다.
