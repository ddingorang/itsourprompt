...
| ~~**Y** — 후반(3라운드 이후) 준수율 하한~~ | ~~70%~~ **미사용** | 중 (유도는 아래 남기지만 **적용할 자리가 없다** — 실측에서 편집이 100% 후반에 몰려 초반 버킷이 비었고, 게이트 2가 폐기됐다. `backend/docs/carry-line-design.md` 9절) |
...
- 출발점은 arXiv:2608.02639의 **지시 1개 준수율 ~96%**다. 우리는 시스템 프롬프트에 한 줄을 더하는
  저부하 구간에 있다.
- 감쇠율은 arXiv:2605.10039의 **OR = 0.944 / step**을 참고한다. 이 계수를 네 스텝에 곱하면
  0.944⁴ ≈ 0.795배이고, 초반 준수율이 0.90(오즈 9)이면 후반 **0.877**, 초반이 0.80(오즈 4)이면
  후반 **0.761**이 된다.
- **다만 이 곱셈은 출처가 명시적으로 부인한 가정 위에 서 있다.** 같은 초록이 그 관계를
  *"non-monotonic rather than a constant per-step effect"* 라고 적었고, 거듭제곱은 정확히
  **일정한 per-step 효과**를 가정한다. 게다가 이 효과는 초록 스스로 *"identified during analysis
  rather than pre-specified"* 라고 밝힌 **사후 탐색 결과**이고, 본문(신뢰구간·비단조성 서술)은
  읽지 못했다(접근 불가 목록 참고). 그러므로 위 숫자들은 **눈금이 아니라 자릿수 감**으로만 쓴다.
- 그 자릿수 감이 말하는 것은 하나다 — **문헌이 잰 세션 내 감쇠는 완만하다.** 70%는 그 완만한
  예측보다 한참 아래에 둔 하한이며, 단위가 다르고(함수 생성 vs 툴 라운드) 모델도 다르므로
  여유를 크게 뒀다. Y가 이렇게 헐거운 이유가 여기 있다.

읽기 보조 하나를 덧붙인다(게이트가 아니다). 하네스가 초반·후반 준수율을 나란히 찍으므로,
**후반/초반 비율이 0.79 미만이면** 원문부터 읽는다. 0.79는 위의 `0.944⁴`에서 온 수인데 그 곱셈이
출처가 부인한 등속 가정 위에 있으므로, 이건 **기준선이 아니라 눈길을 끄는 눈금**이다 — 넘든 못 넘든
판정하지 말고 그 팔의 요약 원문을 읽으라는 뜻으로만 쓴다.
