...
`carry`는 두 렌즈 어느 쪽도 아니다. **LLM이 쓴 문장이 아니라 어템프트를 읽은 순수 계산의 결과**이고, 읽는 사람도
다르다 — 두 렌즈는 사용자에게 말하고 `carry.rule`은 **사용자의 AI에게** 할 지시다. 그래서 마크다운에 섞지 않고
별도 필드로 나가며, 화면에서도 복사해 갈 물건으로 렌더해야 한다.
...
- **`carry`는 저장하지 않고 조회할 때마다 계산한다.** 그래서 이 필드가 생기기 전에 제출된 어템프트도 열면 바로 받는다 — 다른 필드와 달리 "도입 이전" 케이스가 없다. 대신 **같은 어템프트라도 그 사이 실행을 더 돌리면 값이 바뀔 수 있다.**
- **`carry`가 `null`이면 아무것도 렌더하지 않는다.** 빈 절이나 "해당 없음" 자리를 세우지 않는다.
- **두 필드 모두 평문이다** — 다른 피드백 필드와 달리 `Md` 접미사가 없는 것이 그 뜻이다. `rule`은 사용자가 자기 설정 파일에 **그대로 복사해 붙여넣는 문자열**이라 마크업이 섞이면 붙여넣기가 망가지고, `reason`은 코드가 조립하는 한 문장이라 마크업이 생길 자리가 없다. 클라이언트는 Markdown으로 렌더하지 말고 텍스트 그대로 그린다.
- `carry.rule`은 **도구 중립**이다. 파일 이름도 도구 이름도 담지 않으므로 `AGENTS.md`·`CLAUDE.md`·`.cursor/rules` 어디에 붙여넣어도 그대로 쓴다. 파일 이름 권고는 **화면 라벨**에 붙지 규칙 문장에 붙지 않는다.
- 이 봉투는 필드 추가 방식으로만 확장한다. **클라이언트는 모르는 JSON 키를 무시해야 한다.**
...
```json
{
  ...
  "carry": {
    ...
    "rule": "무엇을 바꿨다고만 말하지 마라. 그 변경이 실제로 동작하는지 사람이 확인할 방법을 요약에 반드시 함께 적어라.",
    "reason": "이번 세션의 2턴에서 한 번도 코드를 실행해 결과를 확인하지 않으셨어요. AI가 확인 방법을 요약에 함께 적어 두면 다음엔 무엇을 돌려 봐야 할지 바로 알 수 있어요."
  }
}
```
